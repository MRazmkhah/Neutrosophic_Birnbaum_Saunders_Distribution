# ------------------------------------------------------------------------
# Hazard Function Plot for the Neutrosophic Birnbaum-Saunders Distribution (NBSD)
# ------------------------------------------------------------------------

# -----------------------------
# Hazard Function Definition (H_N)
# -----------------------------
H_N <- function(t, a, b) { 
  # Compute numerator (PDF component)
  u <- (1 / sqrt(2 * pi)) * 
    exp(-1 / (2 * (a^2)) * (t / b + b / t - 2)) * 
    (t^(-3 / 2) * (t + b)) / (2 * a * sqrt(b))
  
  # Compute standard normal argument for denominator
...
# -----------------------------
# Define Parameter Grid and Range
# -----------------------------
t <- seq(0, 2, 0.01)     # Grid of BS variable values
...
# Open Plot Window
# -----------------------------
windows(3, 3)            # For Windows OS users

# -----------------------------
# Compute Initial Hazard Function (Lower Bound)
# -----------------------------
y1 <- H_N(t, a_L, b_L)

# -----------------------------
# Find Max Y-Axis Limit for Plot Scaling
# -----------------------------
...
L <- max(ans1$objective, ans2$objective)  # Max hazard value

# -----------------------------
# Create Plot for Hazard Function
# -----------------------------
plot(t, y1, type = "l", 
     ylab = "Hazard Function", 
     xlab = expression(paste(t[N])), 
     ylim = c(0, L))
axis(side = 1, lwd = 3)
axis(side = 2, lwd = 3)
